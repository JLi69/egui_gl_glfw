    ...
    match event {
        ...
        MouseButton(mouse_btn, glfw::Action::Press, _) => {
            state.input.events.push(egui::Event::PointerButton {
                ...
                button: match mouse_btn {
                    ...
                    glfw::MouseButton::Button4 => egui::PointerButton::Extra2,
                    glfw::MouseButton::Button5 => egui::PointerButton::Extra1,
        ...
        CursorPos(x, y) => {
            state.pointer_pos = pos2(
                x as f32 / state.pixels_per_point,
                y as f32 / state.pixels_per_point,
            );
            state
                .input
                .events
        ...
        Key(keycode, _scancode, glfw::Action::Press, keymod) => {
            use glfw::Modifiers as Mod;
            if let Some(key) = translate_virtual_key_code(keycode) {
                state.modifiers = Modifiers {
                    alt: (keymod & Mod::Alt == Mod::Alt),
        ...
        Key(keycode, _scancode, glfw::Action::Repeat, keymod) => {
            use glfw::Modifiers as Mod;
            if let Some(key) = translate_virtual_key_code(keycode) {
                state.modifiers = Modifiers {
                    alt: (keymod & Mod::Alt == Mod::Alt),
